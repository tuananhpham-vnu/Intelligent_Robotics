...
class LidarImuDataCollector(Node)
    ...
    def qrcode_callback(self, msg)
        ...
        if latency  50
            self.get_logger().warn(fCallback delay is too high {latency.2f}ms)
...
class PatrolNode(BasicNavigator)
    ...
    def init_subscribers(self)
        try
            ...
            self.get_logger().fatal(fSubscription creation failed {str(e)})
            rclpy.shutdown()
            exit(1)
    ...
    def run_voice(self, num)
        file_path = hometurtlebot4training_codevoice
        pygame.mixer.init()
    ...
    def get_and_print_qrcode(self)
        ...
        try
            while rclpy.ok()
                print(ftime{time.perf_counter()-st})
                executor.spin_once(timeout_sec=0.1)
    ...
    def nav_to_point(self, goal)
        ...
        self.get_logger().info(fNavigate to the destination x={goal[0].2f}, y={goal[1].2f}, yaw={goal[2].0f})
        ...
        self.get_logger().error(fNavigation failed {result})
    ...
    def load_navigation_points(self, csv_file_path)
        ...
        try
            ...
            self.get_logger().error(fFailed to load the navigation points {str(e)})
...
def main()
    ...
    wirefram_points = patrol.load_navigation_points(
        csv_file_path='hometurtlebot4training_codepointswirefram_points.csv')
    qr_code_points = patrol.load_navigation_points(
        csv_file_path='hometurtlebot4training_codepointsqr_code_points.csv')
